...
local instructions = { "Defeat FireMan!" }
...
local saveStatePath = "../fcs/MegaMan1.challenge09.fcs"
    ...
    if memory.readbyte(0x06C1) <= 0x0000 then
        ...
        -- Ensure start_frame is not nil before performing arithmetic
            ...
            coreFcn.display_centered_message({
                "Congrats! You completed FireMan!",
...
while true do
    ...
    if memory.readbyte(0x06C1) == 0x001C then
        memory.writebyte(0x06C1, 0x0009)
    end

    coreFcn.display_footer("MegaMan Challenge - Defeat FireMan!")
